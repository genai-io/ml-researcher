...
experiments/
├── README.md         (this file)
├── ledger.tsv        machine-readable log of every experiment iteration
└── EXPxxx_<name>/
    ├── README.md     motivation, parent, hypothesis
    ├── train.py      the training script (the editable file in L1 loop)
...
Tab-separated, append-only log of every iteration. Schema:
...
- The L1 loop advances or resets the branch on each iteration.

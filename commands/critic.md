...
Argument parsing: `$ARGUMENTS` may be `current-best`, `recent`, `report`, or a specific path. Default is `current-best`.
...
1. Resolve scope:
   ...
   - `<path>` → just the named file/directory
...
4. If BLOCK, do NOT proceed with whatever the user was about to do. State: "Critic blocked: <issue>. Fix and rerun /critic before proceeding."
